...
async function startServer() {
  ...
  app.post("/api/system/restore", authenticate, async (req: Request, res: Response) => {
    ...
    const restoredNotifications = req.body.notifications;
    
    console.log(`Restoring ${restoredUsers?.length || 0} users, ${restoredTasks?.length || 0} tasks, ${restoredAttendance?.length || 0} attendance records, and ${restoredNotifications?.length || 0} notifications`);
    ...
    try {
      ...
      if (Array.isArray(restoredNotifications)) {
        notifications.length = 0;
        notifications.push(...restoredNotifications);
      } else {
        notifications.length = 0;
      }
      ...
      res.json({ message: "System restored successfully", userCount: users.length, taskCount: tasks.length, attendanceCount: attendanceRecords.length, notificationCount: notifications.length });
    } catch (err) {
  ...
  app.post("/api/tasks", authenticate, async (req: Request, res: Response) => {
    ...
    const newTask: Task = {
      ...
      points: user.role === 'ENGINEER' ? (urgencyPoints[req.body.urgency as string] || 1) : 0,
    ...
    // Notification Trigger: Engineer assigns to Officer
    if (user.role === 'ENGINEER' && targetUser && targetUser.role === 'OFFICER') {
      notifications.push({
        id: Date.now().toString() + Math.random().toString(36).substr(2, 5),
        userId: targetUser.id,
        message: `Engineer ${user.name} assigned you a new task: ${newTask.title}`,
        type: 'TASK_ASSIGNED',
        read: false,
        timestamp: new Date().toISOString(),
        taskId: newTask.id
  ...
  app.post("/api/attendance", authenticate, async (req: Request, res: Response) => {
    const { technicianId, status, leaveMinutes } = req.body;
    const today = new Date().toISOString().split('T')[0];
    const index = attendanceRecords.findIndex(r => r.technicianId === technicianId && r.date === today);
    if (index !== -1) {
      attendanceRecords[index].status = status;
      if (leaveMinutes !== undefined) attendanceRecords[index].leaveMinutes = leaveMinutes;
    } else {
  ...
  app.post("/api/notifications/read", authenticate, async (req: Request, res: Response) => {
    ...
    notifications.forEach(n => {
      if (n.userId === req.user.id && notificationIds.includes(n.id)) {
        n.read = true;
        updated = true;
      }
    });
  ...
  app.put("/api/tasks/:id", authenticate, async (req: Request, res: Response) => {
    ...
    if (!updatedData.remarks && user.role !== 'SUPER_ADMIN' && !updatedData.assignedTo && !updatedData.title) {
      ...
      if (updatedData.progress === 100 && oldTask.status !== 'COMPLETED') {
        ...
        // Point System: Automatically add point to Assigned Officer Profile
        let officerEmpId = null;
        
        // Case 1: Task was assigned BY an Officer to a Technician
        const assigner = users.find(u => u.employeeId === oldTask.assignedBy);
        if (assigner && assigner.role === 'OFFICER') {
          officerEmpId = assigner.employeeId;
        } 
        // Case 2: Task was assigned TO an Officer by an Engineer
        else {
          const assignee = users.find(u => u.name === oldTask.assignedTo || u.employeeId === oldTask.assignedTo);
          if (assignee && assignee.role === 'OFFICER') {
            officerEmpId = assignee.employeeId;
          }
        }

        if (officerEmpId) {
          const officerIndex = users.findIndex(u => u.employeeId === officerEmpId);
          if (officerIndex !== -1) {
            const taskPoints = oldTask.points || 0;
            users[officerIndex].points = (users[officerIndex].points || 0) + taskPoints;
            console.log(`Added ${taskPoints} points to Officer ${users[officerIndex].name}. Total: ${users[officerIndex].points}`);
          }
        }
      }
  ...
  app.post("/api/tasks/:id/attachments", authenticate, upload.single('attachment'), async (req: Request, res: Response) => {
    const { id } = req.params;
  ...
  app.delete("/api/tasks/:id/attachments/:attachmentId", authenticate, async (req: Request, res: Response) => {
