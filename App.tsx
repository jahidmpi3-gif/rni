...
import { Role, User, Task, Attendance, TaskLog, TaskStatus, TaskAttachment, TaskExtension, Notification } from './types';
...
const CountdownTimer = ({ startedAt, executionTime, extensions, status, compact = false, totalHoldTime = 0, pausedAt }: { startedAt?: string, executionTime?: number, extensions?: any[], status: string, compact?: boolean, totalHoldTime?: number, pausedAt?: string }) => {
  const [timerData, setTimerData] = useState<{ timeLeft: number, progress: number } | null>(null);

  useEffect(() => {
    if (!startedAt || !executionTime || status === 'COMPLETED') {
      setTimerData(null);
      return;
    }
    ...
    const calculateData = () => {
      ...
      const totalExtensions = safeExtensions.reduce((acc, ext) => acc + ext.extraTime, 0);
      const totalAllowedTimeMs = (executionTime + totalExtensions) * 60 * 1000;
      
      let now = new Date().getTime();
      let currentHoldTimeMs = (totalHoldTime || 0) * 60 * 1000;

      // If on hold, use the pausedAt time for calculation to "freeze" the timer
      if (status === 'HOLD' && pausedAt) {
        now = new Date(pausedAt).getTime();
      }

      const timeElapsedMs = now - start - currentHoldTimeMs;
      const diff = totalAllowedTimeMs - timeElapsedMs;
      
      let progress = (timeElapsedMs / totalAllowedTimeMs) * 100;
    ...
    setTimerData(calculateData());
    
    // If on hold, we don't need to update every second
  ...
  }, [startedAt, executionTime, extensions, status, totalHoldTime, pausedAt]);

  if (timerData === null) return null;

  const { timeLeft, progress } = timerData;
  const isExceeded = timeLeft < 0;
  const absTime = Math.abs(timeLeft);
  const minutes = Math.floor(absTime / 60);
  const seconds = absTime % 60;
  const timeString = `${isExceeded ? '-' : ''}${minutes}:${seconds.toString().padStart(2, '0')}`;

  let colorClass = "bg-green-500";
  let textColorClass = "text-green-400";
  let shadowClass = "shadow-[0_0_10px_rgba(34,197,94,0.5)]";

  if (isExceeded) {
    colorClass = "bg-red-500";
    textColorClass = "text-red-500 animate-pulse";
    shadowClass = "shadow-[0_0_15px_rgba(239,68,68,0.6)]";
  } else if (timeLeft < (executionTime || 0) * 60 * 0.2) { // Less than 20% time left
    ...
    return (
      <div className="w-full space-y-1">
        <div className="flex items-center justify-between text-[10px]">
          <div className={cn("font-mono font-bold flex items-center gap-1", textColorClass)}>
            <Clock size={12} />
            {timeString}
          </div>
          <span className={cn("font-bold uppercase", isExceeded ? "text-red-500" : "text-gray-500")}>
            {isExceeded ? "Overtime" : `${Math.round(progress)}%`}
          </span>
        </div>
  ...
  return (
    <div className="w-full space-y-2">
      <div className="flex items-center justify-between">
        <div className={cn("font-mono font-bold text-lg flex items-center gap-2", textColorClass)}>
          <Clock size={18} />
          {timeString}
        </div>
        <span className={cn("text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border", 
          isExceeded ? "border-red-500/50 text-red-500 bg-red-500/10" : "border-white/10 text-gray-400 bg-white/5")}>
          {isExceeded ? "Overtime" : "In Progress"}
        </span>
      </div>
      ...
      <div className="relative w-full h-3 bg-white/5 rounded-full overflow-hidden border border-white/10 p-[1px]">
        <motion.div 
          ...
          animate={{ width: `${progress}%` }}
          transition={{ duration: 1, ease: "linear" }}
          className={cn("h-full rounded-full transition-colors duration-500", colorClass, shadowClass)}
        />
        {/* Progress Markers */}
        <div className="absolute inset-0 flex justify-between px-2 pointer-events-none">
          {[25, 50, 75].map(marker => (
            <div key={marker} className="h-full w-[1px] bg-white/10" />
          ))}
        </div>
      </div>
      
      <div className="flex justify-between text-[8px] text-gray-500 font-medium uppercase tracking-tighter">
        <span>Start</span>
        <span>{Math.round(progress)}% Time Used</span>
        <span>Deadline</span>
      </div>
    </div>
  );
...
const NotificationDropdown = ({ 
  ...
  return (
    <div className="absolute right-0 mt-2 w-80 bg-[#1a1a1f] border border-white/10 rounded-2xl shadow-2xl z-50 overflow-hidden">
          ...
          <div className="divide-y divide-white/5">
            {notifications.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()).map(n => (
              <div 
                ...
                onClick={() => {
                  onMarkAsRead([n.id]);
                  if (n.taskId) onViewTask(n.taskId);
                  onClose();
                }}
              >
                {!n.read && <div className="absolute left-0 top-0 bottom-0 w-1 bg-blue-500" />}
                <p className={cn("text-xs leading-relaxed", !n.read ? "text-white font-medium" : "text-gray-400")}>
                  {n.message}
                </p>
...
export default function App() {
  ...
  const handleUpdateAttendance = async (technicianId: string, status: string, leaveMinutes?: number) => {
    const token = localStorage.getItem('token');
    try {
      await fetchJson('/api/attendance', {
        ...
        body: JSON.stringify({ technicianId, status, leaveMinutes })
      });
      fetchAttendance(token!);
  ...
  const executeRestore = async () => {
    ...
    try {
      ...
      if (systemRestoreInputRef.current) systemRestoreInputRef.current.value = '';
  ...
  const handleAttachmentUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const updatedTask = {
        ...selectedTask,
        attachments: [...(selectedTask.attachments || []), result]
      };
  ...
  const handleDeleteAttachment = async (attachmentId: string) => {
    if (!selectedTask || !window.confirm('Are you sure you want to delete this attachment?')) return;

    setIsLoading(true);
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`/api/tasks/${selectedTask.id}/attachments/${attachmentId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const [newTask, setNewTask] = useState({
    ...
    customStartTime: '',
    estimatedDuration: '',
    executionTime: 30 // Default 30 minutes
  });

  const DUTY_START = "09:00";
  const DUTY_END = "18:00";
  const BASE_WORKING_TIME = 440; // Minutes
  
  const BREAKS = [
    { name: 'Breakfast', start: '10:00', end: '10:15', duration: 15 },
    { name: 'Lunch', start: '13:00', end: '14:00', duration: 60 },
    { name: 'Zuhr + Ozu', duration: 15 },
    { name: 'Asr', duration: 10 }
  ];
  ...
  const handleStatusUpdate = async (e: React.FormEvent) => {
    ...
    if (statusUpdateData.status === 'HOLD') {
      payload.pausedAt = new Date().toISOString();
    } else if (statusUpdateTask.status === 'HOLD' && statusUpdateData.status === 'RUNNING') {
      const pauseTime = new Date(statusUpdateTask.pausedAt || '').getTime();
      const resumeTime = new Date().getTime();
      const holdDurationMins = Math.floor((resumeTime - pauseTime) / (1000 * 60));
      payload.totalHoldTime = (statusUpdateTask.totalHoldTime || 0) + holdDurationMins;
  ...
  const handleTaskSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = editingTask ? {
        ...
        status: (user?.role === 'OFFICER' && staffList.find(s => s.name === newTask.assignedTo)?.role === 'TECHNICIAN') ? 'RUNNING' : editingTask.status,
        startedAt: (user?.role === 'OFFICER' && staffList.find(s => s.name === newTask.assignedTo)?.role === 'TECHNICIAN' && !editingTask?.startedAt) ? new Date().toISOString() : editingTask?.startedAt
      } : {
        ...newTask,
        createdBy: user?.employeeId,
        assignedBy: user?.employeeId,
        status: (user?.role === 'OFFICER' && staffList.find(s => s.name === newTask.assignedTo)?.role === 'TECHNICIAN') ? 'RUNNING' : 
                (staffList.find(s => s.name === newTask.assignedTo)?.supervisorId !== user?.id && user?.role === 'OFFICER' ? 'REQUESTED' : 'PENDING'),
        startedAt: (user?.role === 'OFFICER' && staffList.find(s => s.name === newTask.assignedTo)?.role === 'TECHNICIAN') ? new Date().toISOString() : undefined,
      ...
      setEditingTask(null);
      fetchTasks(token!);
      setNewTask({ title: '', model: 'Portable', details: '', urgency: 'REGULAR', assignedTo: '', deadline: '', points: 1, customStartTime: '', estimatedDuration: '', executionTime: 30 });
    } catch (err) {
  ...
  const [extensionData, setExtensionData] = useState({ extraTime: 30, reason: '' });

  const getTechnicianDailyMinutes = (techId: string) => {
    const today = new Date().toISOString().split('T')[0];
    const techName = staffList.find(s => s.employeeId === techId)?.name;
    if (!techName) return 0;

    const techTasks = tasks.filter(t => 
      t.assignedTo === techName && 
      (t.status === 'COMPLETED' || t.status === 'RUNNING' || t.status === 'HOLD') &&
      (t.startedAt?.startsWith(today) || t.completedAt?.startsWith(today))
    );

    return techTasks.reduce((acc, t) => {
      if (!t.startedAt) return acc;
      
      if (t.status === 'COMPLETED' && t.completedAt) {
        // Reward efficiency: give credit for the full executionTime
        return acc + (t.executionTime || 0);
      } else if (t.status === 'RUNNING') {
        const start = new Date(t.startedAt).getTime();
        const now = new Date().getTime();
        const holdTimeMs = (t.totalHoldTime || 0) * 60 * 1000;
        const elapsed = Math.floor((now - start - holdTimeMs) / (1000 * 60));
        return acc + Math.max(0, elapsed);
      } else if (t.status === 'HOLD' && t.pausedAt) {
        const start = new Date(t.startedAt).getTime();
        const pause = new Date(t.pausedAt).getTime();
        const holdTimeMs = (t.totalHoldTime || 0) * 60 * 1000;
        const elapsed = Math.floor((pause - start - holdTimeMs) / (1000 * 60));
        return acc + Math.max(0, elapsed);
      }
      return acc;
    }, 0);
  ...
  const calculateTechnicianEfficiency = (techId: string) => {
    const today = new Date().toISOString().split('T')[0];
    const techName = staffList.find(s => s.employeeId === techId)?.name;
    if (!techName) return 0;
    ...
    if (techTasks.length === 0) return 0;

    let totalExecutionTime = 0;
    let totalActualTime = 0;

    techTasks.forEach(t => {
      if (t.startedAt && t.completedAt && t.executionTime) {
        const start = new Date(t.startedAt).getTime();
        const end = new Date(t.completedAt).getTime();
        const actualMins = (end - start) / (1000 * 60);
        
        totalExecutionTime += t.executionTime;
        totalActualTime += actualMins;
      }
    });

    if (totalActualTime === 0) return 0;
    return (totalExecutionTime / totalActualTime) * 100;
  };

  const calculateTechnicianDailyPerformance = (workingMinutes: number, leaveMinutes: number = 0) => {
    const baseMinutes = 440 - leaveMinutes;
    if (baseMinutes <= 0) return 0;
    
    // Performance = (Working Minutes / Base Minutes) * 100
    const score = (workingMinutes / baseMinutes) * 100;
  ...
  const handleUpdateTask = async (taskId: string, updates: Partial<Task>) => {
    ...
    try {
      ...
      const newLog: TaskLog = {
        ...
        action: updates.status ? `Status changed to ${updates.status}` : 
                ...
                updates.remarks ? `Remarks updated` : 
                updates.executionTime ? `Execution time set to ${updates.executionTime} mins` : `Task updated`,
        timestamp: new Date().toISOString(),
    ...
    const finalUpdates: any = {
      ...
      if (updates.status === 'HOLD') {
        finalUpdates.pausedAt = new Date().toISOString();
      } else if (currentTask.status === 'HOLD' && updates.status === 'RUNNING') {
        const pauseTime = new Date(currentTask.pausedAt || '').getTime();
        const resumeTime = new Date().getTime();
        const holdDurationMins = Math.floor((resumeTime - pauseTime) / (1000 * 60));
        finalUpdates.totalHoldTime = (currentTask.totalHoldTime || 0) + holdDurationMins;
        finalUpdates.pausedAt = null;
      }

      // Handle cross-team request for Officers
      if (updates.assignedTo && user?.role === 'OFFICER') {
        ...
        } else if (assignee && assignee.role === 'TECHNICIAN') {
          // Auto-start for same-team technicians
      ...
      // Handle Hold Logic
      if (updates.status === 'HOLD' && currentTask.status !== 'HOLD') {
        finalUpdates.pausedAt = new Date().toISOString();
      } else if (updates.status === 'RUNNING' && currentTask.status === 'HOLD' && currentTask.pausedAt) {
        const pauseTime = new Date(currentTask.pausedAt).getTime();
        const resumeTime = new Date().getTime();
        const holdDurationMs = resumeTime - pauseTime;
        const holdDurationMins = holdDurationMs / (1000 * 60);
        finalUpdates.totalHoldTime = (currentTask.totalHoldTime || 0) + holdDurationMins;
        finalUpdates.pausedAt = null; // Reset pausedAt
      }
  ...
  const handleExtendTask = async () => {
    if (!selectedTask || !extensionData.reason.trim()) {
      toast.error('Reason is mandatory for extension');
      return;
    }
    ...
    try {
      const token = localStorage.getItem('token');
      const newExtension: TaskExtension = {
        id: Date.now().toString(),
        extraTime: extensionData.extraTime,
      ...
      const newLog: TaskLog = {
        id: Date.now().toString(),
        action: `Time extended by ${extensionData.extraTime} mins. Reason: ${extensionData.reason}`,
        timestamp: new Date().toISOString(),
        user: user?.name || 'Unknown'
      };
      ...
      const updatedTask = {
        ...selectedTask,
        extensions: [...(selectedTask.extensions || []), newExtension],
      ...
      setExtensionData({ extraTime: 30, reason: '' });
      toast.success('Task time extended successfully');
    } catch (err) {
  ...
  const getAnalyticsData = () => {
    ...
    const trend = Array.from({ length: 7 }).map((_, i) => {
      ...
      const dayTasks = dashboardTasks.filter(t => {
    ...
    const performance = staffList
      ...
      .map(s => ({
        name: s.name.split(' ')[0],
        tasks: dashboardTasks.filter(t => t.assignedTo.toLowerCase() === s.name.toLowerCase() && t.status === 'COMPLETED').length
      }))
    ...
    const modelDistribution = models.map(m => ({
      name: m,
      value: dashboardTasks.filter(t => t.model === m).length
    })).filter(m => m.value > 0);
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                ...
                >
                  ...
                  <div className="p-6 space-y-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-400 mb-2">Execution Time (Minutes)</label>
                      <input 
                        type="number"
                        min="1"
                        placeholder="e.g. 30, 60, 120"
                        value={newTask.executionTime || ''}
                        onChange={(e) => setNewTask({ ...newTask, executionTime: parseInt(e.target.value) || 0 })}
                        className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                      />
                    </div>
                    ...
                    <button 
                      onClick={async () => {
                        if (!selectedTechnician || !newTask.executionTime) {
                          toast.error('Please select a technician and set execution time');
                          return;
                        }
                        ...
                        setNewTask({ ...newTask, executionTime: 0 });
                      }}
                      disabled={!selectedTechnician || !newTask.executionTime || isLoading}
              ...
              <GlassCard className="p-8">
                ...
                <div className="flex flex-col items-center text-center">
                  ...
                  {viewingStaff.role === 'OFFICER' && (
                    <div className="flex items-center gap-3 bg-blue-600/10 p-4 rounded-2xl border border-blue-600/20 mt-4 w-full">
                      <div className="p-3 bg-blue-600/20 rounded-xl text-blue-400">
                        <Award size={24} />
                      </div>
                      <div className="text-left">
                        <p className="text-[10px] text-blue-400 font-bold uppercase tracking-widest">Total Earned Points</p>
                        <p className="text-2xl font-black text-white">{viewingStaff.points || 0}</p>
                      </div>
                    </div>
                  )}
            ...
            >
              <GlassCard className="p-8">
                ...
                <form onSubmit={handleTaskSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-400 mb-2">Deadline Date</label>
                    <input 
                      type={user?.role === 'ENGINEER' ? "date" : "datetime-local"} 
                  ...
                  {['OFFICER', 'HOD', 'SUPER_ADMIN', 'IN_CHARGE'].includes(user?.role || '') && (
                    <div>
                      <label className="block text-sm font-medium text-gray-400 mb-2">Execution Time (Minutes)</label>
                      <input 
                        type="number" 
            ...
            <div className="relative">
              ...
              >
                ...
                {notifications.filter(n => !n.read).length > 0 && (
                  <span className="absolute top-1.5 right-1.5 min-w-[16px] h-4 px-1 bg-red-500 rounded-full border-2 border-[#0a0a0c] text-[8px] font-bold text-white flex items-center justify-center">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-3">
                  {user?.role === 'SUPER_ADMIN' && (
                    <>
                      <button 
                        onClick={() => systemRestoreInputRef.current?.click()}
                          ...
                          <td className="px-4 py-4 min-w-[120px]">
                            {task.status === 'RUNNING' ? (
                              <CountdownTimer 
                                ...
                                status={task.status}
                                compact={true}
                                totalHoldTime={task.totalHoldTime}
                                  ...
                                  <button 
                                    onClick={() => {
                                      ...
                                      setNewTask({
                                        ...
                                        executionTime: task.executionTime || 30
                                      });
                                      setIsTaskModalOpen(true);
                                    }}
                ...
                <div className="flex flex-col items-center text-center">
                  ...
                  {user?.role === 'OFFICER' && (
                    <div className="w-full mt-4">
                      <div className="bg-blue-600/10 p-6 rounded-3xl border border-blue-600/20 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <div className="p-3 bg-blue-600/20 rounded-2xl text-blue-400">
                            <Award size={32} />
                          </div>
                          <div className="text-left">
                            <p className="text-xs text-blue-400 font-bold uppercase tracking-widest">Total Earned Points</p>
                            <p className="text-4xl font-black text-white">{user?.points || 0}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">Rank Status</p>
                          <span className="px-3 py-1 bg-green-500/20 text-green-400 text-[10px] font-bold rounded-full uppercase tracking-widest">Top Performer</span>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <GlassCard key={task.id} className="flex flex-col md:flex-row gap-6 items-center">
                    ...
                    <div className="flex flex-col gap-2 min-w-[180px]">
                      <div className="text-center mb-2">
                        ...
                        <span className={cn(
                          "text-sm font-bold px-3 py-1 rounded-full bg-white/5 border border-white/10",
                          task.status === 'RUNNING' ? "text-amber-500 border-amber-500/20" : "text-blue-400 border-blue-500/20"
                        )}>
                          {task.status}
                        </span>
                      </div>
                  ...
                  .map((tech, i) => {
                    ...
                    return (
                      <GlassCard key={tech.id} className="flex flex-col">
                        ...
                        {techAttendance?.status === 'SHORT_LEAVE' && (
                          <div className="mt-4 pt-4 border-t border-white/5">
                            <label className="block text-[10px] text-gray-500 uppercase mb-2">Leave Duration (Minutes)</label>
                            <input 
                              type="number"
                              value={techAttendance.leaveMinutes || 0}
                              onChange={(e) => {
                                const val = parseInt(e.target.value) || 0;
                                handleUpdateAttendance(tech.employeeId, 'SHORT_LEAVE', val);
                              }}
                              ...
                              {statusInfo.task ? (
                                <div className="space-y-3">
                                  ...
                                  <div className="pt-2 border-t border-white/5">
                                    <CountdownTimer 
                                      startedAt={statusInfo.task.startedAt} 
                                      executionTime={statusInfo.task.executionTime} 
                                      extensions={statusInfo.task.extensions}
                                      status={statusInfo.task.status}
                                      totalHoldTime={statusInfo.task.totalHoldTime}
                                      pausedAt={statusInfo.task.pausedAt}
                                    />
                                  </div>

                                  <div className="grid grid-cols-2 gap-2 pt-1">
                                    <div>
                                      <p className="text-[8px] text-gray-500 uppercase">Work Progress</p>
                                      <p className="text-sm font-bold text-blue-400">{statusInfo.task.progress || 0}%</p>
                                    </div>
                                    <div>
                                      <p className="text-[8px] text-gray-500 uppercase">Efficiency</p>
                                      <p className="text-sm font-bold text-blue-400">
                                        {calculateTechnicianEfficiency(tech.employeeId).toFixed(1)}%
                                      </p>
                                    </div>
                                  </div>
                                  <div className="pt-2 border-t border-white/5">
                                    <div className="flex justify-between items-center">
                                      <p className="text-[8px] text-gray-500 uppercase">Daily Performance</p>
                                      <p className="text-sm font-bold text-green-400">
                                        {(() => {
                                          const today = new Date().toISOString().split('T')[0];
                                          const techAttendance = attendance.find(a => a.technicianId === tech.employeeId && a.date === today);
                                          const leaveMins = techAttendance?.status === 'SHORT_LEAVE' ? (techAttendance.leaveMinutes || 0) : 0;
                                          return calculateTechnicianDailyPerformance(getTechnicianDailyMinutes(tech.employeeId), leaveMins).toFixed(1);
                                        })()}%
                                      </p>
                                    </div>
                                  </div>
                            ...
                            {engineerTasks.filter(t => t.status === 'RUNNING').map(task => (
                              <div key={task.id} className="p-3 rounded-xl bg-blue-500/5 border border-blue-500/10 hover:border-blue-500/20 transition-all cursor-pointer group" onClick={() => { setSelectedTask(task); setIsTaskDetailsModalOpen(true); }}>
                                ...
                                <div className="flex items-center justify-between mt-2 pt-2 border-t border-white/5">
                                  <span className="text-[9px] text-blue-400 font-bold">Work: {task.progress || 0}%</span>
                                  <span className="text-[9px] text-gray-400">Started: {task.startedAt ? new Date(task.startedAt).toLocaleTimeString() : 'N/A'}</span>
                                </div>
                              </div>
                            ))}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <GlassCard className="p-4 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-4xl font-black">{dashboardTasks.length}</p>
                  </GlassCard>
                  <GlassCard className="p-4 flex flex-col items-center justify-center text-center">
                    <p className="text-xs text-gray-500 uppercase tracking-widest mb-2">Completion Rate</p>
                    <p className="text-4xl font-black text-green-500">
                      {dashboardTasks.length > 0 ? Math.round((dashboardTasks.filter(t => t.status === 'COMPLETED').length / dashboardTasks.length) * 100) : 0}%
                    </p>
                  </GlassCard>
                  <GlassCard className="p-4 flex flex-col items-center justify-center text-center">
                    <p className="text-xs text-gray-500 uppercase tracking-widest mb-2">Avg. Progress</p>
                    <p className="text-4xl font-black text-blue-500">
                      {dashboardTasks.length > 0 ? Math.round(dashboardTasks.reduce((acc, t) => acc + t.progress, 0) / dashboardTasks.length) : 0}%
                    </p>
                  </GlassCard>
                  <GlassCard className="p-4 flex flex-col items-center justify-center text-center">
                ...
                >
                  <GlassCard className="p-8">
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                          ...
                          <div className="flex flex-col gap-3">
                            <div className="flex items-center gap-3">
                              <p className={cn(
                                "text-sm font-bold px-3 py-1 rounded-full bg-white/5 border border-white/10",
                                selectedTask.status === 'HOLD' ? "text-red-400 border-red-500/20" : 
                                selectedTask.status === 'COMPLETED' ? "text-green-400 border-green-500/20" : "text-blue-400 border-blue-500/20"
                              )}>
                                {selectedTask.status}
                              </p>
                            </div>
                            <div className="p-4 bg-white/5 rounded-2xl border border-white/5 space-y-4">
                              <div className="flex items-center justify-between">
                                <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest">Time Progress</p>
                                <CountdownTimer 
                                  startedAt={selectedTask.startedAt} 
                                  executionTime={selectedTask.executionTime} 
                                  extensions={selectedTask.extensions}
                                  status={selectedTask.status}
                                  compact={true}
                                  totalHoldTime={selectedTask.totalHoldTime}
                              ...
                              <div className="space-y-1">
                                <div className="flex justify-between text-[10px] text-gray-500 uppercase font-black tracking-widest">
                                  ...
                                  <span className="text-blue-400">{selectedTask.progress || 0}%</span>
                                </div>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 p-6 bg-white/5 rounded-2xl border border-white/10">
                        <div>
                          <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-1">Start Time</p>
                          <p className="text-sm font-bold text-blue-400">{selectedTask.startedAt ? new Date(selectedTask.startedAt).toLocaleString() : 'Not Started'}</p>
                        </div>
                        <div>
                          <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-1">Deadline Date</p>
                          <p className="text-sm font-bold text-red-400">{new Date(selectedTask.deadline).toLocaleString()}</p>
                        </div>
                        <div>
                          <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-1">Assigned Time</p>
                          <p className="text-sm font-bold text-amber-400">{selectedTask.executionTime ? `${selectedTask.executionTime} Minutes` : 'N/A'}</p>
                        </div>
                        <div>
                          <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-1">Remaining Time</p>
                        ...
                        <div>
                          <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-1">Time Taken</p>
                          <p className="text-sm font-bold text-green-400">
                            {selectedTask.status === 'COMPLETED' 
                              ? (selectedTask.taskTakenTime || calculateDuration(selectedTask.startedAt, selectedTask.completedAt))
                              : 'In Progress'}
                          </p>
                        </div>
                        <div>
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          <p className="text-xs text-gray-400 uppercase tracking-widest">Attachments</p>
                          <button 
                            onClick={() => taskAttachmentInputRef.current?.click()}
                            className="p-1 hover:bg-white/5 rounded-lg text-blue-400 transition-all flex items-center gap-1 text-[10px] font-bold"
                          >
                            <Plus size={14} />
                            Add Attachment
                          </button>
                        ...
                        <div className="space-y-2 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                          {selectedTask.attachments?.map((attachment) => (
                            <div key={attachment.id} className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/10 group">
                              <div className="flex items-center gap-3 overflow-hidden">
                                ...
                                <div className="overflow-hidden">
                                  ...
                                  <p className="text-[10px] text-gray-500">
                                    {attachment.uploadedBy} • {new Date(attachment.uploadedAt).toLocaleString()}
                                  </p>
                                </div>
                              </div>
                          ...
                          {(!selectedTask.attachments || selectedTask.attachments.length === 0) && (
                            <p className="text-xs text-gray-500 italic">No attachments added yet.</p>
                          )}
                        </div>
                      </div>
                      ...
                      {selectedTask.extensions && selectedTask.extensions.length > 0 && (
                        <div>
                          <p className="text-xs text-gray-400 uppercase tracking-widest mb-2">Extension History</p>
                          <div className="space-y-2">
                            {selectedTask.extensions.map((ext) => (
                              <div key={ext.id} className="text-[10px] p-2 rounded-lg bg-amber-500/5 border border-amber-500/10">
                                <div className="flex justify-between items-center mb-1">
                                  <span className="text-amber-400 font-bold">+{ext.extraTime} mins by {ext.user}</span>
                                  <span className="text-gray-500">{new Date(ext.timestamp).toLocaleString()}</span>
                                </div>
                                <p className="text-gray-300">Reason: {ext.reason}</p>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {user?.role === 'OFFICER' && selectedTask.status !== 'COMPLETED' && (
                        <div className="pt-4 border-t border-white/10 space-y-4">
                          <div className="flex items-center justify-between">
                            <label className="block text-xs text-gray-400 uppercase tracking-widest">Time Control</label>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              <label className="block text-[10px] text-gray-500 uppercase mb-1">Execution Time (Mins)</label>
                              <input 
                                type="number" 
                                value={newTask.executionTime}
                                onChange={(e) => setNewTask({...newTask, executionTime: parseInt(e.target.value) || 0})}
                            ...
                            <div>
                              <label className="block text-[10px] text-gray-500 uppercase mb-1">Assign to Technician</label>
                              <div className="flex gap-2">
                                ...
                                <button 
                                  ...
                                  className="bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 text-white font-bold px-4 py-2 rounded-xl transition-all text-xs"
                                >
                                  Assign
                                </button>
                              </div>
        ...
        {isExtensionModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-md bg-[#0f0f12] border border-white/10 rounded-3xl p-8 shadow-2xl overflow-hidden"
            >
              ...
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setIsExtensionModalOpen(false)} className="text-gray-500 hover:text-white">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="space-y-6">
                <div>
                  <label className="block text-xs text-gray-400 uppercase tracking-widest mb-2">Extra Time (Minutes)</label>
                  <div className="grid grid-cols-4 gap-2">
                    {[15, 30, 60, 120].map(mins => (
                      <button
                        key={mins}
                        onClick={() => setExtensionData({...extensionData, extraTime: mins})}
                        className={cn(
                          "py-2 rounded-xl text-xs font-bold transition-all border",
                          extensionData.extraTime === mins 
                  ...
                  <input 
                    type="number"
                    value={extensionData.extraTime}
                    onChange={(e) => setExtensionData({...extensionData, extraTime: parseInt(e.target.value) || 0})}
                    className="w-full mt-2 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-amber-500/50"
                    placeholder="Custom minutes..."
                  />
                </div>

                <div>
                  <label className="block text-xs text-gray-400 uppercase tracking-widest mb-2">Reason for Extension (Mandatory)</label>
                  <textarea 
                    value={extensionData.reason}
                    onChange={(e) => setExtensionData({...extensionData, reason: e.target.value})}
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-amber-500/50 h-32"
                ...
                <button 
                  ...
                  disabled={isLoading || !extensionData.reason.trim()}
                  className="w-full bg-amber-600 hover:bg-amber-700 disabled:bg-gray-600 text-white font-bold py-4 rounded-2xl transition-all shadow-lg shadow-amber-900/20"
                >
                  Confirm Extension
                </button>
              </div>
